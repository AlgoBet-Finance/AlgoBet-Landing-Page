  ...
  return (
    <Box style={{ maxWidth: '800px', margin: 'auto' }}>
      ...
      {arrayTrackingTokens.map(({ address, token }) => {
        ...
        return (
          <p key={address}>
            balance: {tokenBalance ? getFullDisplayBalance(new BigNumberJS(tokenBalance)) : '--'} {token.symbol}
          </p>
        )
      })}
      ...
      <p>b3: send ETH to another address</p>
      ...
      <select
        ...
        onChange={(e) =>
          setSelectedCurrency(
            e.target.value === 'ETH' ? Currency.ETHER : deserializeToken(trackingTokens[e.target.value]!)
          )
        }
      >
